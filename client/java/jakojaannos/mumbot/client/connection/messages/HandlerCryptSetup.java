...
public class HandlerCryptSetup implements TcpMessageHandler.IHandler<Mumble.CryptSetup> {
    ...
    public void handle(MumbleClient client, Mumble.CryptSetup message) {
        System.out.println("Received crypt setup.");
        System.out.println("  -> hasClientNonce: " + message.hasClientNonce());
        if (message.hasClientNonce())
        {
            System.out.printf("   --> Size: %d\n", message.getClientNonce().size());
            System.out.printf("   --> Bytes: ");

            for (byte b : message.getClientNonce().toByteArray())
                System.out.printf("%d " , b);
            System.out.println();
        }

        System.out.println("  -> hasServerNonce: " + message.hasServerNonce());
        if (message.hasServerNonce())
        {
            System.out.printf("   --> Size: %d\n", message.getServerNonce().size());
            System.out.printf("   --> Bytes: ");

            for (byte b : message.getServerNonce().toByteArray())
                System.out.printf("%d " , b);
            System.out.println();
        }
